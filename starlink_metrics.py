...
THREAT_SEVERITY_MULTIPLIERS = {"low": 0.5, "medium": 1.0, "high": 2.0}
...
class AlertThresholds:
    ...
    def __post_init__(self):
        ...
        if not (
            self.critical_stability < self.degraded_stability < self.stable_stability
        ):
            raise ValueError(
                "Thresholds must be in ascending order: critical < degraded < stable"
            )
...
class StarlinkConnectionQuality:
    ...
    def __init__(
        ...
        self.stability_history: deque = deque(
            maxlen=history_window_size if history_window_size > 0 else None
        )

    @overload
    def calculate_quality_score(
        self, return_details: Literal[False] = False
    ) -> float: ...

    @overload
    def calculate_quality_score(
        self, return_details: Literal[True]
    ) -> QualityScoreDetails: ...

    def calculate_quality_score(
        self, return_details: bool = False
    ) -> Union[float, QualityScoreDetails]:
        """
        ...
        if len(self.active_threats) > 0:
            ...
            logger.info(
                ...
                len(self.active_threats),
                total_threat_deduction,
            )
        ...
        if deductions_list and logger.isEnabledFor(logging.DEBUG):
            deduction_summary = ", ".join(
                f"{d['reason']}: {d['points']}" for d in deductions_list
            )
            logger.debug(
                "Quality score deductions: %s. Final score: %s",
                deduction_summary,
                final_score,
            )
    ...
    def _calculate_stability(self, packet_loss: float, latency: float) -> float:
        ...
        loss_factor = max(
            0, 1 - packet_loss * self.stability_thresholds.packet_loss_multiplier
        )
        # Latency factor based on configurable maximum
        latency_factor = max(0, 1 - latency / self.stability_thresholds.max_latency)
    ...
    def calculate_stability_score(self, use_smoothing: bool = True) -> float:
        ...
        current_stability = self._calculate_stability(
            packet_loss_decimal, self.metrics.latency
        )
...
def monitor_connection(
    packet_loss: float, latency: float, active_threats: Optional[List[Any]] = None
) -> dict:
    """
