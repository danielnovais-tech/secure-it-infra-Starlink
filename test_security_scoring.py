...
class TestConfigValidation(unittest.TestCase):
    ...
    def test_valid_config(self):
        ...
        config = {"multipliers": {"critical": 0.7, "elevated": 0.9, "normal": 1.0}}
        # Should not raise
    ...
    def test_invalid_security_level(self):
        ...
        config = {"multipliers": {"invalid_level": 0.5}}
        with self.assertRaises(ConfigValidationError) as context:
            validate_config_schema(config)
    ...
    def test_negative_multiplier(self):
        ...
        config = {"multipliers": {"critical": -0.5}}
        with self.assertRaises(ConfigValidationError) as context:
            validate_config_schema(config)
    ...
    def test_non_numeric_multiplier(self):
        ...
        config = {"multipliers": {"critical": "not_a_number"}}
        with self.assertRaises(ConfigValidationError) as context:
            validate_config_schema(config)
...
class TestSecurityScorer(unittest.TestCase):
    ...
    def test_config_file_multipliers(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            config = {"multipliers": {"critical": 0.6, "elevated": 0.85, "normal": 1.0}}
            json.dump(config, f)
            config_path = f.name
    ...
    def test_invalid_config_file_uses_defaults(self):
        ...
        scorer = SecurityScorer(
            SecurityLevel.CRITICAL, config_file="/nonexistent/config.json"
        )
        result = scorer.calculate_score(100.0)
    ...
    def test_invalid_config_raises_error(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            config = {"multipliers": {"critical": -0.5}}  # Negative multiplier
            json.dump(config, f)
            config_path = f.name
...
class TestIntegration(unittest.TestCase):
    ...
    def test_end_to_end_with_config_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            config = {"multipliers": {"critical": 0.6, "elevated": 0.8, "normal": 1.0}}
            json.dump(config, f)
            config_path = f.name
